...
class Model(nn.Module):
    def __init__(self, opt, num_frame=9, num_joints=17, in_chans=2,
    ...
    def forward(self, x):
        ...
        # Collapse temporal and DCT dims → final feature vector
        x = torch.cat((self.weighted_mean(x[:, :self.num_coeff_kept]),
                   self.weighted_mean_(x[:, self.num_coeff_kept:])), dim=-1)
        
        embedding = x.clone()              # save the high-level embedding

        pose_3d = self.head(x).view(b, 1, p, -1, 1).permute(0,3,1,2,4)

        return pose_3d, embedding  # [160, 3, 1, 17, 1], [160, 3, 81, 17, 1]
...
"""    def forward(self, x):
        ...
        x = self.head(x).view(b, 1, p, -1, 1).permute(0, 3, 1, 2, 4)
        return x, None  # [160, 3, 1, 17, 1], [160, 3, 81, 17, 1]"""